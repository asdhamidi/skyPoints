...
| `demo/generate_sample_feed.py` | Python script | Produces per-country fixture files for a given run date: a small fixed set of "golden" rows carrying the known edge cases (ambiguous USA dates, AUS `"NULL"` string, IND `Individual or Corporate` values, a tracked-attribute change across two run dates), plus `--record-count` (default 250,000) deterministic bulk rows per source in disjoint ID ranges, for volume | `--run-date`, `--record-count` | `landing/*.xlsx`, `landing/*.csv`, `landing/*.json` |
...
| `stg_member_profile_aus.sql` | dbt model (incremental, `unique_key='member_key'`) | Harmonizes `RAW.AUS_MEMBER_PROFILE` into the canonical schema; `TRY_TO_DATE` handling for `"NULL"`/invalid strings; only new `RAW` rows processed per run (docs/01 §11) | `RAW.AUS_MEMBER_PROFILE` | canonical-shape rows, `country_code='AUS'` |
| `stg_member_profile_ind.sql` | dbt model (incremental, `unique_key='member_key'`) | Harmonizes `RAW.IND_MEMBER_PROFILE`; parses `M/D/YYYY`; maps `Individual or Corporate` → `membership_type` | `RAW.IND_MEMBER_PROFILE` | canonical-shape rows, `country_code='IND'` |
| `stg_member_profile_usa.sql` | dbt model (incremental, `unique_key='member_key'`) | Harmonizes `RAW.USA_MEMBER_PROFILE`; parses concatenated dates deterministically only (via `parse_usa_date`), quarantines ambiguous ones to `NULL`; `dob` always `NULL` | `RAW.USA_MEMBER_PROFILE` | canonical-shape rows, `country_code='USA'` |
...
| `stg_redemptions.sql` | dbt model (incremental, `unique_key='txn_id'`) | Flattens `RAW.REDEMPTION_FEED.payload:redemptions` via `LATERAL FLATTEN`; only new `RAW` rows processed per run (docs/01 §11) | `RAW.REDEMPTION_FEED` | `STAGING.REDEMPTIONS` |
...
| `generate_country_tables.sql` (model) | dbt model | Thin wrapper that invokes the `generate_country_tables` macro as a side effect of `dbt run --select marts`; own output is an audit row only. The macro now issues `CREATE TABLE IF NOT EXISTS ... LIKE` once plus a `MERGE` every run, not `CREATE OR REPLACE` (docs/01 §11) | `int_member_profile_final`, `country_reference` | `MARTS.TABLE_<COUNTRY>` per country + an audit row |
...
| `redemptions.sql` | dbt model (incremental, `unique_key='txn_id'`) | Joins `stg_redemptions` to `int_member_profile_final` on `member_id`; classifies each row `RESOLVED`/`AMBIGUOUS`/`ORPHAN` rather than assuming a clean join (docs/01 §7); global (not split by country); only new transactions processed per run, each classified against the full current population (docs/01 §11) | both models above | `MARTS.REDEMPTIONS` |
